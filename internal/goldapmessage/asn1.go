...
// sizeInt64 计算整数的最小化有符号 BER 编码长度（本地副本修复：原实现
// 对 [128,255] 等正数漏算前导 0x00 符号字节，导致 messageID≥128 被写成负数）。
...
func writeInt64(bytes *Bytes, i int64) (size int) {
	// writeBytes 为前插语义：先写低字节最终呈大端序（与原实现一致）。
	// 修复点：按 sizeInt64 的正确长度补前导符号字节（正值 0x00 / 负值 0xFF）。
	n := sizeInt64(i)
	bodyLen := 0
	for j := i; j != 0 && j != -1 || bodyLen == 0; j >>= 8 {
		bytes.writeBytes([]byte{byte(j)})
		bodyLen++
	}
	for k := bodyLen; k < n; k++ {
		if i >= 0 {
			bytes.writeBytes([]byte{0x00})
		} else {
			bytes.writeBytes([]byte{0xFF})
		}
	}
	return n
}
...
// writeUintBody 写无符号整数内容字节（低→高，前插语义呈大端序），供长度字段使用。
func writeUintBody(bytes *Bytes, u uint64) (size int) {
	for ; u != 0 || size == 0; u >>= 8 {
		bytes.writeBytes([]byte{byte(u)})
		size++
	}
	return
}
...
func writeTagAndLength(bytes *Bytes, t TagAndLength) (size int) {
	...
	if t.Length < 0 {
		...
		// 长度为无符号域：直接最小字节大端，无符号前导（本地副本修复：
		// 此处曾复用有符号 writeInt64，长度 128-255 会被多补 0x00 导致 size/write 错位）
		lengthBytes := writeUintBody(bytes, uint64(t.Length))
